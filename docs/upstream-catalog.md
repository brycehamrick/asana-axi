...
How to list asana-axi in the shared AXI catalog on [axi.md](https://axi.md).
Do this only AFTER the npm package is published and dogfooded (soak period),
per the agreed sequence. One-time per release; the entry stays.

Source: <https://github.com/kunchenguid/axi/blob/main/CONTRIBUTING.md>

## Preconditions

- [ ] `asana-axi` published on npm and installable via `npx -y asana-axi@latest`
- [ ] Public repo live at <https://github.com/brycehamrick/asana-axi>
...
2. Append one entry to the `community` list in `catalog.yaml` (keep it in
   ...
   ```yaml
     - name: asana-axi
       url: https://github.com/brycehamrick/asana-axi
       author: brycehamrick
       domain: Project management
       description: "Create, list, view, edit, move, comment on, and tag Asana tasks through the REST API with token-efficient TOON output and idempotent mutations."
   ```
...
5. Commit exactly the three generated files (`catalog.yaml`, `README.md`,
   `docs/index.html`) with a conventional message (`docs: add asana-axi to
   catalog`). Never hand-edit the generated table regions.
